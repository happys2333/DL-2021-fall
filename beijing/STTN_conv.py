...
class ConvTransAttention(nn.Module):
    def __init__(self,k,heads=4, embed_size=64):
        super(ConvTransAttention, self).__init__()
        ...
        self.conv_Q = nn.Conv2d(self.embed_size,self.embed_size,(1,self.k),stride=1)
        self.conv_K = nn.Conv2d(self.embed_size,self.embed_size,(1,self.k),stride=1)
        # self.conv = nn.Conv2d(1,1,(1,k),stride=1)
        
    def forward(self, input_Q, input_K, input_V):
        #         torch.Size([32, 207, 12, 64])
#         _b, _n, _t, _c = input_Q.shape
#         _conv = nn.Conv2d(_c,_c,(1,self.k),stride=1)
        
        
        input_Q = input_Q.permute(0,3,2,1)
        input_Q = F.pad(input_Q,(int(self.k-1),0))
        input_Q = self.conv_Q(input_Q)
        input_Q = input_Q.permute(0,3,2,1)
        
        input_K = input_K.permute(0,3,2,1)
        input_K = F.pad(input_K,(int(self.k-1),0))
        input_K = self.conv_K(input_K)
        input_K = input_K.permute(0,3,2,1)
        
        B, N, T, C = input_V.shape
        c_B, c_N, c_T, c_C = input_Q.shape
#         print(input_Q.shape,input_V.shape)

        # [B, N, T, C] --> [B, N, T, h * d_k] --> [B, N, T, h, d_k] --> [B, h, T, N, d_k]
        Q = self.W_Q(input_Q).view(c_B, c_N, c_T, self.heads, self.head_dim).transpose(1, 3)  # Q: [B, h, T, N, d_k]
        K = self.W_K(input_K).view(c_B, c_N, c_T, self.heads, self.head_dim).transpose(1, 3)  # K: [B, h, T, N, d_k]
...
#         self.attention = SMultiHeadAttention(embed_size, heads)
        self.attention = ConvTransAttention(5,heads,embed_size)
