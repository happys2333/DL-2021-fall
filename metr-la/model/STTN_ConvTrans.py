...
class STransformer(nn.Module):
    def __init__(self, embed_size, heads, adj, cheb_K, dropout, forward_expansion):
        ...
        self.context_embedding = context_embedding(64, 64, 5)
    ...
    def forward(self, value, key, query):
        ...
        q1 = query.permute(0, 1, 3, 2).reshape(B * N, C, T)
        #         print('q1: ',q1.shape)
        C_S = self.context_embedding(q1)
        #         print('C_T: ',C_T.shape)
        C_S = C_S.reshape(B, N, C, T).permute(0, 1, 3, 2)

        assert C_S.shape == D_S.shape
...
class TTransformer(nn.Module):
    ...
    def forward(self, value, key, query, t):
        ...
        #         print('expanded D_T: ',D_T.shape)

        q1 = query.permute(0, 1, 3, 2).reshape(B * N, C, T)
        #         print('q1: ',q1.shape)
        C_T = self.context_embedding(q1)
        #         print('C_T: ',C_T.shape)
        C_T = C_T.reshape(B, N, C, T).permute(0, 1, 3, 2)

        assert C_T.shape == D_T.shape
